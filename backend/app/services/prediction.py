...
# 24: starter rates use consistent units, recent form is no longer double-counted, confirmed
# lineups gate the PA engine, MLB PA extras use the automatic runner, and calibration stores
# pre/post distribution diagnostics. Older replays must be regenerated under those semantics.
SIMULATION_SUMMARY_SCHEMA_VERSION = 24
...
def predict_game(game: Any, home: Any, away: Any, home_pitcher: Any | None, away_pitcher: Any | None,
    ...
    model_name = (model_runtime or {}).get("model_name") or (prediction_context or {}).get("model_name") or (
        "KBO_MATCHUP_V15" if game.league == "KBO" else "MLB_MATCHUP_V14"
    )
...
def _apply_daily_bullpen_workload(staff: dict[str, Any], workload: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for tier in output["bullpen"]:
        output["bullpen"][tier] = round(min(1.8, float(output["bullpen"][tier]) * (1 + .10 * fatigue)), 4)
