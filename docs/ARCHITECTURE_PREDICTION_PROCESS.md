...
시장 핸디캡·총점은 점수 평균이나 승률 평균을 직접 움직이는 입력이 아니다. 모델 분포에서
독립적으로 공정 핸디·공정 총점과 적중 확률을 계산한 뒤, 배당사 실제 기준점·가격과의 차이
및 경기 종료 후 결과를 `DerivedMarketHistory`에 누적한다. 이력 표본이 충분하고 시장 대비
Brier 성능 개선이 확인되기 전에는 보정값으로 사용하지 않는다.
...
- 시장 가격은 `T-24h`, `T-3h`, `T-60m`, `T-15m`별 불변 snapshot으로 남긴다. 가격이
  그대로여도 체크포인트 행을 저장해 미수집과 무변동을 구분한다.
- 원배당과 무배당 확률을 함께 저장한다. Brier·Log Loss에는 무배당 확률을, ROI에는
  실제 decimal odds를, CLV에는 당시 가격과 경기 전 마지막 가격을 사용한다.
- 득점 잔차는 초반 선발 구간과 후반 불펜 구간으로 나누며, 반복 근거가 없는 큰 잔차는
  다음 경기 평균에 더하지 않고 분산 후보로 보낸다.
- 시뮬레이션은 정배의 0~2득점 패배, 5회 리드 후 역전패, 역배의 5회 리드 유지 승리를
  별도 경로 확률로 기록하고 `favorite_fragility`는 이 경로를 설명만 한다.
- 시장 log-odds offset 모델과 시점×정배강도 보정은 shadow로 저장한다. 각각 가격 보유
  300경기와 시점 라벨 보유 검증 표본을 채우고 Brier·Log Loss를 모두 개선하기 전에는
  운영 승률을 바꾸지 않는다.
- paired ablation은 동일 경기·동일 난수로 운영/잔차 제거/시장 제거/역배 변동성 제거를
  비교한다. 승률뿐 아니라 득점 MAE, -1.5 Brier, 총점 Brier를 함께 본다.
- 역배 신호의 `READY` 조건은 가격 보유 언더독 300경기, 4%p 이상 edge 75경기, 시장 대비
  Brier·Log Loss 개선, 양의 단위수익률, 음수가 아닌 closing-line value를 모두 요구한다.
