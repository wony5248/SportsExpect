...
7. 2026-08-23 KBO 경기부터 경기 전 기준 기대득점과 실제 득점의 팀별 공격·수비 잔차를 6경기 반감기 EWMA로 계산한다. 홈·원정 잔차는 별도로 축소하고, 맞대결 잔차는 24경기 사전표본으로 강하게 축소한 뒤 10%만 반영한다.
8. 팀별 잔차 변동성은 리그 잔차 분산으로 축소해 홈·원정 시뮬레이션 분산에 각각 반영한다. 타석 엔진은 자체 타석 변동성이 있으므로 팀 분산의 35%만 추가한다.
...
잔차 보정 비교도 매 경기보다 먼저 종료된 경기만 순차 누적해 원 기대득점 대비 보정 기대득점의 MAE·RMSE와 승률 Brier/Log Loss/calibration error를 함께 반환한다. MLB는 과거 예측-결과 표본이 충분해질 때까지 잔차 보정을 비활성화한다.
...
- 경기 시작 3시간 전부터 시작 후 6시간까지 5분 간격 상태·결과 안전망 갱신
- 경기 시작 4시간 후 결과 재확인
