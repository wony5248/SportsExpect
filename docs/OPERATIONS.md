...
화면 상단 `Claude 설정`에서 `ADMIN_TOKEN`과 Claude API 키를 입력하고 `키 인증 · 모델 불러오기`를 누르면 Models API가 이 계정에서 사용 가능한 모델을 조회합니다. 모델과 활성 여부를 선택해 저장하면 `runtime_secrets`에 키는 암호화되고 모델은 설정값으로 저장됩니다. 이후에는 원문 키를 다시 입력하지 않고 모델이나 활성 상태만 변경할 수 있습니다. 브라우저 저장소와 API 응답에는 원문 키를 남기지 않습니다. 암호화에는 `SECRET_ENCRYPTION_KEY`를 사용하며, 비어 있으면 충분히 긴 `ADMIN_TOKEN`을 사용합니다. 변경 내용은 다음 Cron 또는 수동 예측 갱신부터 적용됩니다.
