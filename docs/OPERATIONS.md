...
Supabase Cron
  ...
  → MLB 다음 날 일정을 12:50 KST에 선취득하고 13:00 KST에 저장 데이터 기본 예측 생성
  → 22:55 KST 일정 재확인 후 23:00 KST에 최대 5경기 단위 제한 청크로 최신 외부 데이터 재계산
...
- API-Sports Baseball 배당: KBO 12:00 KST, MLB 21:00 KST 기준 각 1회. 하루 한 번 갱신되는 피드이므로 성공한 슬롯은 다시 호출하지 않고 앱 상한은 하루 20요청
- MLB 21시 작업: 다음 날 일정·선발과 배당을 갱신한 뒤, 선발 등 입력이 바뀐 경기만 예측 재계산
